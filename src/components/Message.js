...
const Message = ({ id, message, timestamp, name, email, photoURL }) => {
  ...
  return (
    <div className="flex items-center p-1 pl my-5 mr-2 hover:bg-discord_messageBg group">
      <img
        src={photoURL}
        alt=""
      ...
      {user?.email === email && (
        <div
          className="hover:bg-discord_deleteIcon p-1 ml-auto rounded-sm text-discord_deleteIcon hover:text-white cursor-pointer"
          onClick={() =>
            db
              .collection("channels")
              .doc(channelId)
